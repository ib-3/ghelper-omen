...
namespace OmenCore.Hardware
{
    public class ThermalSensorProvider
    {
        ...
        public IEnumerable<TemperatureReading> ReadTemperatures()
        {
            ...
            // Try LibreHardwareMonitor first for CPU and GPU if direct bridge is available
            if (_bridge != null)
            {
                cpuTemp = _bridge.GetCpuTemperature();
                gpuTemp = _bridge.GetGpuTemperature();
            }
            // Use WmiBiosMonitor's cached readings (which are LHM-backed when override is active)
            else if (_wmiBiosMonitor != null)
            {
                try
                {
                    ...
                    bool onUiThread = System.Windows.Forms.Application.MessageLoop;
                    if (onUiThread)
                    {
                        ...
                        catch (TimeoutException)
                        {
                            ...
                            System.Diagnostics.Debug.WriteLine("[ThermalSensorProvider] UI-thread ReadSampleAsync timed out (250ms)");
                        }
                    }
                    else
                    {
                ...
                catch (Exception ex)
                {
                    ...
                    System.Diagnostics.Debug.WriteLine($"[ThermalSensorProvider] ReadSampleAsync failed: {ex.Message}");
                }
            }
            ...
            // Ultimate ACPI fallback if LHM / WMI BIOS returned no temperature
            if (cpuTemp <= 0)
            {
                cpuTemp = AcpiThermalZoneTemperatureReader.ReadCpuTemperature(ref _cpuThermalZoneInstance);
            }
