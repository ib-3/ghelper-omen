...
namespace OmenCore.Hardware
{
    ...
    public class WmiFanController : IDisposable
    {
        ...
        private System.Threading.Timer? _countdownExtensionTimer;
        ...
        public WmiFanController(LibreHardwareMonitorImpl? hwMonitor, LoggingService? logging = null, int maxFanLevelOverride = 0, IHpWmiBios? injectedWmiBios = null, bool isDesktop = false)
        {
            ...
            //   Desktop  → 100 (percentage-based WMI, mirrors HP's GetPlatformSettingsType() Desktop path)
            //   Laptop V2→ 56  (krpm range, mirrors HP's NotebookV2 path)
            //   Laptop V1→ 55  (classic krpm, mirrors HP's NotebookV0/V1 path)
            if (_wmiBios is HpWmiBios concrete)
            {
                concrete.DetectMaxFanLevel(maxFanLevelOverride, isDesktop: isDesktop);
            }
            _maxFanLevel = _wmiBios.MaxFanLevel;
            _logging?.Info($"WmiFanController: Max fan level = {_maxFanLevel}{(maxFanLevelOverride > 0 ? $" (user override: {maxFanLevelOverride})" : $" (auto-detected, isDesktop={isDesktop})")}");
        }
        
        // Helper methods for getting sensor data with WMI BIOS fallback
        private double GetCpuTemperature()
        {
            double acpiTemp = AcpiThermalZoneTemperatureReader.ReadCpuTemperature(ref _cpuThermalZoneInstance);
            if (acpiTemp > 0) return acpiTemp;
            ...
            try
            {
                ...
                if (isMaxPreset)
                {
                    ...
                    if (_wmiBios.SetFanMax(true))
                    {
                        ...
                        if (VerifyMaxAppliedWithRetries(out var maxTelemetryUnavailable))
                        {
                        ...
                        if (maxTelemetryUnavailable)
                        {
                            _logging?.Warn("SetFanMax succeeded but fan telemetry is unavailable - accepting command and maintaining max mode with keepalive");
                            MarkMaxModeAccepted();
                            return true;
                        }
        ...
        private void MarkMaxModeAccepted()
        {
        ...
        public bool ApplyCustomCurve(IEnumerable<FanCurvePoint> curve)
        {
            ...
            try
            {
                ...
                // NOTE: 0% is intentionally allowed here as a real manual level (0 RPM).
                // The BIOS keepalive timer (line ~1657) re-sends SetFanLevel(0,0) on each tick
                // so the BIOS will not bounce back to auto-mode. If your firmware wedges at
                // 0 RPM and does not recover, use the Auto preset instead of setting 0% in a curve.
                if (targetPoint.FanPercent < 0)
                {
                    _logging?.Warn($"Custom curve point has negative FanPercent ({targetPoint.FanPercent}) at {maxTemp}°C - ignoring.");
                    return false;
                }
        ...
        public bool SetFanSpeeds(int cpuPercent, int gpuPercent)
        {
            ...
            // the BIOS from reverting to auto-mode. Use RestoreAutoControl() explicitly
            // if you want to hand back control to BIOS thermal management.
            ...
            try
            {
                ...
                else
                {
                    ...
                    // SetFanLevel(0, 0) is a valid WMI command on both V1 (krpm scale) and
                    // V2 (percentage scale) firmware. The keepalive timer re-sends it every
                    // ~2.5s so the BIOS does not bounce back to auto after its 120s timeout.
        ...
        /// </summary>
        private bool VerifyMaxAppliedWithRetries(out bool telemetryUnavailable)
        {
            ...
            {
                _logging?.Warn($"[VerifyMax] Fan telemetry unavailable after {attempts} attempts - cannot verify readback");
                return false;
            }
        ...
        public void StartCountdownExtension()
        {
            lock (_countdownTimerLock)
            {
                ...
                _countdownExtensionTimer = new System.Threading.Timer(CountdownExtensionCallback, null,
