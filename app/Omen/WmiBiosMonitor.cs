...
namespace OmenCore.Hardware
{
    ...
    ///
    /// Data Sources:
    /// - CPU Temperature: ACPI thermal zone, then HP WMI BIOS / worker fallback
    /// - GPU Temperature: HP WMI BIOS / NVAPI
    ...
    public class WmiBiosMonitor : IHardwareMonitorBridge, IDisposable
    {
        ...
        private string _cpuTemperatureSource = "fallback";
        private string? _lastLoggedCpuTemperatureSource;
        ...
        // First call to LHM may take several seconds for hardware scanning/initialization.
        // Start with a generous 5s timeout, then tighten to 1.5s once we confirm LHM is responsive.
        private int _cpuFallbackReadTimeoutMs = 5000;
        private const int CpuFallbackReadCooldownSeconds = 5;  // was 30s — retry LHM sooner after a timeout
        ...
        private DateTime _lastGpuPowerCheckUtc = DateTime.MinValue;
        private static readonly TimeSpan GpuInactivePowerCheckInterval = TimeSpan.FromSeconds(120);
        private bool _gpuInactiveSkipLogged;
        ...
        public WmiBiosMonitor(LoggingService? logging = null, NvapiService? nvapi = null, PawnIOMsrAccess? msrAccess = null)
        {
            ...
            _workerBackedCpuTempOverrideEnabled = true; // Always prioritize highly-precise LHM core/package temperature by default
            ...
            // Removed EnsureTempFallbackMonitor() pre-warm because initializing LHM (which uses HidSharp) 
            // before the Application message loop is fully started causes "HidSharp RegisterClass failed" crash.
        ...
        private void UpdateReadings()
        {
            ...
            try
            {
                ...
                // ═══════════════════════════════════════════════════════════════
                // SOURCE 2: HP WMI BIOS — GPU temp + fan RPM, CPU temp fallback
                // CPU temp is used only when ACPI is unavailable/invalid.
                // ═══════════════════════════════════════════════════════════════
                ...
                // ── GPU-inactive fast path ──────────────────────────────────
                // When the discrete GPU has been inactive for several readings
                // (set by SanitizeGpuTelemetry), skip ALL NVAPI calls entirely
                // to let the GPU stay in D3/RTD3 sleep. Use only Windows GPU
                // Engine counters (no NVAPI, no GPU wake) to detect when the
                // GPU becomes active again.
                bool skipNvapiThisCycle = false;
                if (HardwareControl.IsEcoMode())
                {
                    skipNvapiThisCycle = true;
                    _cachedGpuTemp = 0;
                    _cachedGpuLoad = 0;
                ...
                else if (_gpuInactive)
                {
                    var gpuEngineLoad = TryReadWindowsGpuEngineLoadPercent();
                    bool hasWinGpuActivity = gpuEngineLoad > 1.0;
                    bool cooldownElapsed = (DateTime.UtcNow - _lastGpuPowerCheckUtc) >= GpuInactivePowerCheckInterval;

                    if (hasWinGpuActivity)
                    {
                        // Windows says the GPU is active again — allow NVAPI to re-probe
                        _gpuInactive = false;
                        _consecutiveGpuInactiveReads = 0;
                        _gpuInactiveSkipLogged = false;
                        _logging?.Info($"[WmiBiosMonitor] GPU woke up (WinGpuEngine load={gpuEngineLoad:F1}%) — resuming NVAPI monitoring");
                    }
                    else if (!cooldownElapsed)
                    {
                        // GPU still sleeping, cooldown not elapsed — zero everything, skip NVAPI
                        skipNvapiThisCycle = true;
                        _cachedGpuTemp = 0;
                        _cachedGpuLoad = 0;
                        _cachedGpuPowerWatts = 0;
                        ...
                        if (!_gpuInactiveSkipLogged)
                        {
                            _logging?.Info("[WmiBiosMonitor] GPU inactive — skipping all NVAPI calls to allow D3/RTD3 sleep");
                            _gpuInactiveSkipLogged = true;
                        }
                    }
                    else
                    {
                        // Cooldown elapsed, no Windows activity — do one NVAPI probe
                        // to confirm GPU is still asleep, then reset the timer
                        _lastGpuPowerCheckUtc = DateTime.UtcNow;
                        _logging?.Debug("[WmiBiosMonitor] GPU inactive cooldown elapsed — allowing one NVAPI re-probe");
                    }
                }

                // Try Afterburner shared memory first (eliminates NVAPI contention)
                if (!skipNvapiThisCycle && _afterburnerService?.IsMsiAfterburnerSharedMemoryAvailable == true)
                {
                    try
                    {
                ...
                if (!skipNvapiThisCycle && !afterburnerProvidedData && _nvapi?.IsAvailable == true && !_nvapiMonitoringDisabled)
                {
                    try
                    {
                        _lastGpuPowerCheckUtc = DateTime.UtcNow;
                        if (!_nvapi.IsGpuPowered())
                        {
                            _cachedGpuTemp = 0;
                            _cachedGpuLoad = 0;
                            _cachedGpuPowerWatts = 0;
        ...
        private bool ApplyCpuTemperature(double cpuTemp, string source, int? freezeThreshold = null)
        {
            if (cpuTemp <= 0 || cpuTemp >= 110)
            {
                return false;
            }

            int threshold = freezeThreshold ?? ((_cachedCpuLoad < 20 && _cachedCpuPowerWatts <= 15)
                ? IdleConsecutiveIdenticalTempReads
                : MaxConsecutiveIdenticalTempReads);

            if (Math.Abs(cpuTemp - _lastCpuTempReading) < 0.1)
            {
                _consecutiveIdenticalCpuTempReads++;
                if (_consecutiveIdenticalCpuTempReads > threshold && !_cpuTempFrozen)
                {
                    var nowUtc = DateTime.UtcNow;
                    if ((nowUtc - _lastCpuFreezeWarnAt).TotalSeconds >= FreezeWarnCooldownSeconds)
                    {
                        ...
                        _logging?.Warn($"[WmiBiosMonitor] CPU temperature source {source} appears frozen at {cpuTemp:F1}C for {_consecutiveIdenticalCpuTempReads} readings (load={_cachedCpuLoad:F0}%, power={_cachedCpuPowerWatts:F1}W)");
                    }
                }
            }
            else
            {
                ...
                if (_cpuTempFrozen)
                {
                    ...
                    _logging?.Info($"[WmiBiosMonitor] CPU temperature source {source} recovered after {frozenDuration.TotalSeconds:F0}s freeze");
            ...
            LogCpuTemperatureSource(source, cpuTemp);
            return true;
        }

        private void LogCpuTemperatureSource(string source, double tempC)
        {
            if (string.Equals(_lastLoggedCpuTemperatureSource, source, StringComparison.OrdinalIgnoreCase))
            {
                return;
            }

            _lastLoggedCpuTemperatureSource = source;
            _logging?.Info($"[WmiBiosMonitor] CPU temp source: {source} ({tempC:F1}C)");
        }
        ...
        private void TryApplyCpuTemperatureFallback()
        {
            ...
            bool acpiPrimaryHealthy = string.Equals(_cpuTemperatureSource, "ACPI", StringComparison.OrdinalIgnoreCase) &&
                                      _cachedCpuTemp > 0 &&
                                      !_cpuTempFrozen &&
                                      !lowAndLoaded &&
                                      !lowAndHighPower;

            // Force LHM query if available; fall back to WMI/ACPI if primary is unhealthy
            ...
            try
            {
                ...
                if (!fallbackTask.Wait(TimeSpan.FromMilliseconds(_cpuFallbackReadTimeoutMs)))
                {
                    _cpuFallbackReadDisabledUntilUtc = DateTime.UtcNow.AddSeconds(CpuFallbackReadCooldownSeconds);
                    if (!_cpuFallbackTimeoutLogged)
                    {
                        _logging?.Warn($"[WmiBiosMonitor] CPU temp fallback timed out after {_cpuFallbackReadTimeoutMs}ms — disabling fallback for {CpuFallbackReadCooldownSeconds}s to keep monitoring responsive");
                ...
                if (fallbackCpuTemp > 0 && fallbackCpuTemp < 110)
                {
                    ...
                    // LHM hardware scanning is done — tighten timeout for subsequent reads
                    if (_cpuFallbackReadTimeoutMs > 1500)
                        _cpuFallbackReadTimeoutMs = 1500;
                    ...
                    if (shouldApplyFallback)
                    {
                        ...
                        _cpuTemperatureSource = "LHM/worker";
                        LogCpuTemperatureSource(_cpuTemperatureSource, fallbackCpuTemp);
                        ...
                        {
                            _logging?.Warn($"[WmiBiosMonitor] CPU temp fallback active: ACPI/HP WMI reading looked invalid ({previous:F1}C), using LibreHardwareMonitor ({fallbackCpuTemp:F1}C)");
        ...
        private static bool ShouldPreferWorkerCpuTemp(string model)
        {
            ...
            return model.Contains("OMEN MAX 16", StringComparison.OrdinalIgnoreCase) ||
                   ...
                   model.Contains("14-fb0", StringComparison.OrdinalIgnoreCase) ||
        ...
        {
            // Always use LibreHardwareMonitor for CPU package power as requested
            bool cpuNeedsFallback = true; 
        ...
        private LibreHardwareMonitorImpl? EnsureTempFallbackMonitor()
        {
            ...
            try
            {
                ...
                // WinUI startup currently keeps configuration outside the copied hardware layer.
                // Use safe defaults until the full settings service is ported.
                ...
                _tempFallbackMonitor = new LibreHardwareMonitorImpl(
                    ...
                    // In eco/optimus mode, skip GPU hardware enumeration entirely.
                    // LHM opening GPU handles wakes the dGPU from RTD3 sleep.
                    disableGpu: HardwareControl.IsEcoMode());
