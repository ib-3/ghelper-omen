...
namespace GHelper
{
    public partial class Extra : RForm
    {
        ...
        public Extra()
        {
            ...
            panelOmenMaxFan.Visible = AppConfig.IsOmen();
            
            int maxRpm = AppConfig.Get("omen_max_fan_level", 0);
            if (maxRpm <= 0) maxRpm = 60;
            numericOmenMaxFan.Value = maxRpm;
            numericOmenMaxFan.ValueChanged += (s, e) =>
            {
                AppConfig.Set("omen_max_fan_level", (int)numericOmenMaxFan.Value);
                if (Program.settingsForm != null && Program.settingsForm.fansForm != null && Program.settingsForm.fansForm.Visible)
                {
                    Program.settingsForm.fansForm.InitFans();
                }
            };
        ...
        private void InitUxtuSettings()
        {
            ...
            if (Application.ExecutablePath.EndsWith("dev.exe", StringComparison.OrdinalIgnoreCase))
            {
                CheckBox checkDevShowZoned = new CheckBox();
                checkDevShowZoned.Text = "DEV: Show Zoned Toggle on Per-Key Keyboards";
                checkDevShowZoned.AutoSize = true;
                checkDevShowZoned.Margin = new Padding(12, 5, 0, 0);
                checkDevShowZoned.Checked = AppConfig.Is("omen_dev_show_zoned");
                checkDevShowZoned.CheckedChanged += (s, e) => { AppConfig.Set("omen_dev_show_zoned", checkDevShowZoned.Checked ? 1 : 0); };
                panelSettings.Controls.Add(checkDevShowZoned);
            }
        }
