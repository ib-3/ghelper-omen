...
namespace OmenCore.Hardware
{
    ...
    public class HpWmiBios : IHpWmiBios, IDisposable
    {
        ...
        private bool QuerySystemData()
        {
            ...
            try
            {
                ...
                if (result != null && result.Length >= 9)
                {
                    ...
                    {
                        // Check model name for OMEN Max or Transcend which may report V1 but need V2
                        var modelName = GetModelName();
                        if (!string.IsNullOrEmpty(modelName) && 
                            ((modelName.Contains("MAX", StringComparison.OrdinalIgnoreCase) && modelName.Contains("OMEN", StringComparison.OrdinalIgnoreCase)) ||
                             modelName.Contains("Transcend", StringComparison.OrdinalIgnoreCase)))
                        {
                            _logging?.Info($"  ⚠️ OMEN Max or Transcend detected by name but reports V1 - forcing V2 for fan commands");
        ...
        public void DetectMaxFanLevel(int userOverride = 0, int? modelMaxFanLevel = null, bool isDesktop = false)
        {
            try
            {
                ...
                // HP separation: Desktop vs Laptop (mirrors HP's GetPlatformSettingsType()).
                // HP desktops (OMEN 25L/30L/35L/40L/45L) use a 0-100 percentage-based WMI
                // fan control range — their firmware accepts 0-100 directly with no hardware cap.
                // Laptops use the krpm level scale (0-55 classic, 0-56 on V2 BIOS).
                if (isDesktop)
                {
                    MaxFanLevel = 100;
                    _logging?.Info($"Max fan level set to {MaxFanLevel} (Desktop — percentage range)");
                    return;
                }

                // Notebook/Laptop path — use ThermalPolicy to pick the right hardware scale.
                // v2.8.6: Removed current-fan-level auto-detection heuristic that caused false
                ...
                // V0/V1 thermal policy — classic 0-55 krpm range. Matches HP's NotebookV0/V1 path.
                MaxFanLevel = 55;
                _logging?.Info($"Max fan level set to {MaxFanLevel} (Laptop, classic krpm range)");
            }
            catch (Exception ex)
            {
                MaxFanLevel = 55;
