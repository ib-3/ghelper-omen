...
namespace OmenCore.Hardware
{
    ...
    public class LibreHardwareMonitorImpl : IHardwareMonitorBridge, IDisposable
    {
        ...
        private bool _disableGpu = false; // When true, skips GPU handle enumeration (eco/optimus mode)
        ...
        /// </summary>
        public LibreHardwareMonitorImpl(Action<string>? logger = null, IMsrAccess? msrAccess = null, bool disableGpu = false)
        {
            _logger = logger;
        ...
        public LibreHardwareMonitorImpl(Action<string>? logger, bool useWorker, IMsrAccess? msrAccess = null, bool orphanTimeoutEnabled = true, int orphanTimeoutMinutes = 5, bool disableGpu = false)
        {
            _logger = logger;
            _msrAccess = msrAccess;
        ...
        private void InitializeComputer()
        {
            try
            {
                ...
                {
                    IsCpuEnabled = true,
                    // Skip GPU hardware enumeration in eco/optimus mode — opening GPU handles
                    // wakes the discrete GPU from D3/RTD3 sleep even when only CPU temp is needed.
                    IsGpuEnabled = !_disableGpu,
                    IsMemoryEnabled = true,
                    IsStorageEnabled = true,
                    IsControllerEnabled = false, // DISABLED to prevent HidSharp RegisterClass crashes
                ...
                foreach (var hardware in _computer?.Hardware ?? Array.Empty<IHardware>())
                {
                    ...
                    switch (hardware.HardwareType)
                    {
                        case HardwareType.Cpu:
                            ...
                            // "Core Max" = max junction temp across all cores, matches HWInfo/PowerShell exactly.
                            // Package/DTS sensors read 10-15°C lower than true core temps on modern Intel CPUs.
                            var cpuTempSensor = GetSensorExact(hardware, SensorType.Temperature, "Core Max")         // Max junction across all cores (most accurate)
                                ?? GetSensor(hardware, SensorType.Temperature, "Core (Tctl/Tdie)")                  // AMD Ryzen primary (Tdie = junction)
                                ?? GetSensor(hardware, SensorType.Temperature, "Tctl/Tdie")                         // AMD Ryzen (partial match)
                                ?? GetSensor(hardware, SensorType.Temperature, "Core DTS")                          // Intel DTS (Arrow Lake / Core Ultra)
                                ?? GetSensorExact(hardware, SensorType.Temperature, "CPU Package")                  // Intel Package (lower than Core Max)
                                ?? GetSensor(hardware, SensorType.Temperature, "Package")                           // Intel Package (partial)
                                ?? GetSensorExact(hardware, SensorType.Temperature, "CPU MAX")                      // Alternate Core Max label
                            ...
                            _logger?.Invoke($"CPU temp sensor selected: {cpuTempSensor?.Name ?? "none"} = {rawCpuTemp:F1}°C");
