...
namespace OmenCore.Hardware
{
    ...
    public class FanControllerFactory
    {
        ...
        public WmiFanControllerWrapper? TryCreateWmiController()
        {
            try
            {
                // Pass isDesktop so WmiFanController picks the correct MaxFanLevel:
                //   Desktop (OMEN 25L/30L/35L/40L/45L) → 100 (percentage-based WMI)
                //   Laptop V2 (most modern OMEN notebooks) → 56 (0-56 krpm hardware range)
                //   Laptop V0/V1 (classic OMEN notebooks)  → 55 (0-55 krpm hardware range)
                bool isDesktop = _capabilities?.IsDesktop ?? false;
                var controller = new WmiFanController(_libreHwMonitor!, _logging, _maxFanLevelOverride, isDesktop: isDesktop);
                if (controller.IsAvailable)
                {
                    var wrapper = new WmiFanControllerWrapper(controller, _logging);
                    if (!wrapper.TestCommandEffectiveness())
                    {
                        _logging?.Warn("WMI commands appear ineffective. Falling back...");
                        wrapper.Dispose();
                        return null;
                    }
                    return wrapper;
    ...
    internal static class SensorHelper
    {
        ...
        private static string? _cpuThermalZoneInstance;
        ...
        public static IEnumerable<(string Name, double Rpm)> GetFanSpeeds(LibreHardwareMonitorImpl? libreHw)
        {
            ...
            // Fall back to WMI BIOS (only V2+ supports direct RPM correctly)
            var result = new List<(string, double)>();
            if (WmiBios.ThermalPolicy >= HpWmiBios.ThermalPolicyVersion.V2)
            {
        ...
        public static double GetCpuTemperature(LibreHardwareMonitorImpl? libreHw)
{
    ...
    if (wmiTemp.HasValue && wmiTemp.Value > 0) return wmiTemp.Value;

    // ACPI last — known to report wrong values on Arrow Lake
